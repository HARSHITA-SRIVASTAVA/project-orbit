...
export default function OrbitStatus() {
  return (
    <div className="relative overflow-hidden rounded-[2rem] border border-blue-100 bg-white p-8 shadow-[0_24px_80px_rgba(37,99,235,0.08)]">
      ...
      <div className="relative">
        ...
        <div className="mt-8 divide-y divide-slate-100">
          {statusItems.map((item) => (
            <div key={item.label} className="flex gap-5 py-5 first:pt-0 last:pb-0">
              <div
                className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-slate-50 text-lg font-bold ${item.accent}`}
              >
                {item.icon}
              </div>
